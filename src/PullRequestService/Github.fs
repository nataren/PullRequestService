...
type t(owner, token) =
    ...
    member this.ProcessMergedPullRequest (prMetadata : MindTouch.Domain.MergedPullRequestMetadata) =
        ...
        (if DateTime.UtcNow.Subtract(new TimeSpan(7, 0, 0)) > release then
            let mergingMessage = String.Format("PR #{0}: Auto-merging {1} to {2}", prMetadata.PrNumber, prMetadata.Head?label.AsString(), "master")
       ...
       |> Seq.iter (fun (src, target) ->
                        let mergingMessage = String.Format("PR #{0}: Auto-merging {1} to {2}", prMetadata.PrNumber, src, target)
                        logger.Debug (repo + ": " + mergingMessage)
